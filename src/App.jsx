...
const ResultsScreen = ({ category, onBack, onSelectSupplier }) => {
  const suppliers = [
    ...
    { id: 2, name: "Maria Lopez",  role: category, priceFrom: 320, rating: 4.9, jobs: 203, img: "https://i.pravatar.cc/100?img=45" },
    { id: 3, name: "DJ Nova",      role: category, priceFrom: 280, rating: 4.7, jobs: 98,  img: "https://i.pravatar.cc/100?img=23" },
  ];
  return (
    <Phone>
        ...
        {suppliers.map((s) => (
          <Card key={s.id}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "Poppins, sans-serif", fontWeight: 800 }}>{s.name}</div>
                <div style={{ fontSize: 12, color: Brand.muted }}>{s.role} • ⭐ {s.rating} ({s.jobs})</div>
  ...
  return (
    <Phone>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <img src={supplier?.img || `https://i.pravatar.cc/120?u=${supplier?.name}`} alt={supplier?.name} style={{ width: 96, height: 96, borderRadius: 999, objectFit: "cover" }} />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 22, fontWeight: 900, fontFamily: "Poppins, sans-serif", color: Brand.text }}>{supplier?.name}</div>
        ...
        <div style={{ fontSize: 18, fontWeight: 900, fontFamily: "Poppins, sans-serif", color: Brand.text }}>Bio</div>
        <Card>Experienced {supplier?.role} for weddings, parties, and corporate events.</Card>

        <div style={{ fontSize: 18, fontWeight: 900, fontFamily: "Poppins, sans-serif", color: Brand.text }}>Services Offered</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {packages.map((p) => (
            <div key={p.tier} style={{ ...styles.card, padding: 14 }}>
              <div style={{ fontSize: 16, fontWeight: 900, fontFamily: "Poppins, sans-serif" }}>{p.tier}</div>
              <div style={{ marginTop: 6, fontWeight: 800 }}>€ {p.price}<span style={{ fontSize: 12, color: Brand.muted }}>{p.unit}</span></div>
            </div>
          ))}
        </div>

        <div style={{ fontSize: 18, fontWeight: 900, fontFamily: "Poppins, sans-serif", color: Brand.text }}>Availability</div>
        ...
        <div style={{ fontSize: 18, fontWeight: 900, fontFamily: "Poppins, sans-serif", color: Brand.text }}>Portfolio</div>
        <Card>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ marginTop: 6 }}>Absolutely fantastic! Kept the dance floor packed all night.</div>
            </div>
            <img src="https://maps.geoapify.com/v1/staticmap?style=osm-carto&width=140&height=120&center=lonlat:-0.1276,51.5072&zoom=11&apiKey=demo" alt="London map" style={{ width: 140, height: 120, objectFit: "cover", borderRadius: 12 }} />
          </div>
        </Card>

        <div style={{ display: "flex", gap: 8 }}>
          <Button style={{ flex: 1, background: Brand.navy }}>Message</Button>
          <Button style={{ flex: 1, background: Brand.orange }} onClick={() => onBook("Silver (€300)")}>Book Now</Button>
        </div>
...
const BookingPage = ({ supplier = "George Harris", selectedPackage = "Silver (€300)", onBack, onSelectTab }) => {
  ...
  return (
    <Phone>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, flex: 1 }}>
        <Card>
          <div style={{ fontSize: 18, fontWeight: 800, fontFamily: "Poppins, sans-serif" }}>{supplier}</div>
          <div style={{ fontSize: 14, marginTop: 4, color: Brand.muted }}>Package: {selectedPackage}</div>
        </Card>

        <div style={{ fontSize: 16, fontWeight: 800, fontFamily: "Poppins, sans-serif" }}>Choose Date</div>
        <input type="date" value={date} onChange={(e) => setDate(e.target.value)} style={{ ...styles.card, padding: 10, fontSize: 14 }} />

        <div style={{ fontSize: 16, fontWeight: 800, fontFamily: "Poppins, sans-serif" }}>Choose Time</div>
        <input type="time" value={time} onChange={(e) => setTime(e.target.value)} style={{ ...styles.card, padding: 10, fontSize: 14 }} />

        <div style={{ fontSize: 16, fontWeight: 800, fontFamily: "Poppins, sans-serif" }}>Notes</div>
        <textarea placeholder="Add any special requests (optional)" value={notes} onChange={(e) => setNotes(e.target.value)} style={{ ...styles.card, padding: 10, fontSize: 14, minHeight: 80, resize: "vertical" }} />

        <Button
          style={{ width: "100%", marginTop: 12, background: Brand.teal }}
          onClick={() => {
            const payload = { id: Date.now(), supplier, selectedPackage, date, time, notes, status: "Confirmed", createdAt: new Date().toISOString() };
            try {
...
const BookingsTab = ({ onSelectTab }) => {
  ...
  useEffect(() => {
    try { setBookings(JSON.parse(localStorage.getItem("aeh_bookings") || "[]")); } catch { setBookings([]); }
  }, []);

  const cancel = (id) => {
    const updated = bookings.map((b) => (b.id === id ? { ...b, status: "Cancelled" } : b));
    setBookings(updated);
    try { localStorage.setItem("aeh_bookings", JSON.stringify(updated)); } catch {}
  };
  ...
  return (
    <Phone>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>
        {bookings.length === 0 && (
          <Card>
            <div style={{ fontWeight: 800, marginBottom: 6, fontFamily: "Poppins, sans-serif" }}>No bookings yet</div>
            <div style={{ fontSize: 13, color: Brand.muted }}>When you confirm a booking it will appear here.</div>
            <Button style={{ marginTop: 10, background: Brand.orange }} onClick={() => onSelectTab?.("home")}>Find services</Button>
          </Card>
        )}
        {bookings.map((b) => (
          <Card key={b.id}>
            <div style={{ fontFamily: "Poppins, sans-serif", fontWeight: 900 }}>{b.supplier}</div>
            <div style={{ fontSize: 12, color: Brand.muted }}>{b.selectedPackage}</div>
            <div style={{ fontSize: 12 }}>{b.date} at {b.time}</div>
            <span style={{ fontSize: 12, fontWeight: 800, color: b.status === "Cancelled" ? Brand.red : Brand.teal }}>{b.status}</span>
            <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
              <Button style={{ background: Brand.navy, flex: 1 }} onClick={() => alert(JSON.stringify(b, null, 2))}>View</Button>
              <Button style={{ background: Brand.red, flex: 1 }} onClick={() => cancel(b.id)}>Cancel</Button>
            </div>
          </Card>
        ))}
        <BottomTabs active="bookings" onSelect={onSelectTab} />
  ...
  return (
    <Phone>
      ...
      <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>
        <div>
          <div style={{ fontWeight: 800, fontFamily: "Poppins, sans-serif", marginBottom: 6 }}>Your Name</div>
          <input style={{ ...styles.input, width: "100%" }} value={name} onChange={(e) => setName(e.target.value)} placeholder="Jane Doe" required />
        </div>
        <div>
          <div style={{ fontWeight: 800, fontFamily: "Poppins, sans-serif", marginBottom: 6 }}>Email</div>
          <input type="email" style={{ ...styles.input, width: "100%" }} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" required />
        </div>
        <div>
          <div style={{ fontWeight: 800, fontFamily: "Poppins, sans-serif", marginBottom: 6 }}>Message</div>
          <textarea style={{ ...styles.textarea, width: "100%" }} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="How can we help?" required />
        </div>
        <Button type="submit" style={{ background: Brand.orange, width: "100%" }}>Send</Button>
